...
const API = (() => {
  ...
  return {
    ...
    // Vannes
    sendVanne: (targetId, message_key) => request('POST', `/users/${targetId}/send-vanne`, { message_key }),
    getVannes: (userId) => request('GET', `/users/${userId}/vannes`),
    markVannesRead: (userId) => request('PATCH', `/users/${userId}/vannes/read`, {}),
  };
})();
